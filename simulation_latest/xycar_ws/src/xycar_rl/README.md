...
ros2 launch xycar_rl real_shadow.launch.py \
  ...
  drive_enabled:=false deployment_speed_cap:=10.0 \
  max_inference_rate_hz:=0.0 \
  lidar_safety_enabled:=false steering_gain:=1.0 device:=cpu
```

The real-car launch default limits `deployment_speed_cap` to `10`. Keep this
cap for model comparisons; `0.0` disables the additional cap.
...
- Real-car model runs use `deployment_speed_cap=10` unless an explicit lower
  cap is required for a test.
