...
보행자 회피, 앞차 추월, 신호등, 미션 전환, 속도 결정, 긴급 정지와 최종 `/xycar_motor` 발행은 rule-based 코드가 담당합니다. 이 패키지의 recorder와 학습 코드는 `/xycar_motor`를 발행하지 않습니다.
...
출력 범위는 `-1~1`이며 실제 조향각은 학습에 사용한 `max_steer_deg`를 곱해 복원합니다. 실차 추론 노드와 최종 모터 발행 코드는 이 패키지에 포함되지 않습니다.
