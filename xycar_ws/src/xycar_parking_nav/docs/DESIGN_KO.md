...
보정값 기준 약 0.322 m/s다. Nav2 평균속도를 전·후진 0.16 m/s로 제한해 펄스
duty가 50%를 넘지 않게 하며, 20 Hz 루프에서 연속 ON은 최대 0.10초 뒤 반드시
0이 된다. 펄스 ON 순간속도 0.322 m/s, 연속 ON 0.10초, 감속 0.80 m/s^2에서
계산한 정지거리 약 0.097 m보다 긴 0.14 m까지 곡선 swept rectangle을 검사한다.
Nav2 제한과 별개로 최종 모터 어댑터도 평균 명령을 2 이하로 포화시켜, 외부의
잘못된 `/cmd_vel` 입력이 연속 명령 4로 바뀌는 것을 막는다.

실차가 명령 4 미만에서 움직이지 않으므로 Nav2가 요구하는 저속을 명령 4로 계속
올려 보내지 않는다. 20 Hz 출력 루프에서 누적된 목표 command-time과 실제
command-time 차이를 계산하고, 20 Hz 한 주기인 0.05초 이상의 `4/0` 또는
`-4/0` 펄스를 발생시켜
시간 평균속도를 맞춘다. 정지·방향전환·조향정렬·LiDAR 충돌·센서 timeout 때는
누적 펄스 상태를 초기화하고 즉시 0을 출력한다. 실차 주차 launch는 VESC 내부의
두 번째 acceleration slew를 끄므로 짧은 4/0 펄스가 소실되지 않는다. 펄스용
속도·안전 설정만 바꾸고, 기존에 전체 완주를 검증한 31단계 좌표와 공식
`B_PARK (2.1, 3.3, -pi/2)` Pose는 변경하지 않는다.
