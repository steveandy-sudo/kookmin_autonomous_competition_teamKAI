#!/usr/bin/env python3
"""Initialize AMCL by matching LiDAR along a known route."""
...
from .route_localization_core import route_prefix
...
class RouteScanLocalizer(Node):
    ...
    def _declare_parameters(self) -> None:
        ...
        self.declare_parameter("initial_route_search_distance_m", -1.0)
    ...
    def _on_path(self, message: PathMessage) -> None:
        ...
        search_distance = float(
            self.get_parameter("initial_route_search_distance_m").value
        )
        self.route_points = route_prefix(points, search_distance)
        if not self.initial_pose_published:
            self._publish_status(
                f"WAIT_SCAN route_points={len(self.route_points)}/"
                f"{len(points)} search_distance={search_distance:.2f}m"
            )
    ...
    def _publish_initial_pose(self, match: RouteMatch) -> None:
        if self.initial_pose_pub.get_subscription_count() < 1:
            self._publish_status("WAIT_AMCL_INITIALPOSE_SUBSCRIBER")
            return
