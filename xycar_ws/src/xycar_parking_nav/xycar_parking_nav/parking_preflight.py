...
class ParkingPreflight(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("lidar_device", "/dev/ttyLIDAR")
        self.declare_parameter("imu_device", "/dev/ttyIMU")
        self.declare_parameter("vesc_device", "/dev/ttyMOTOR")
        ...
        self.probes = {
            "scan": TopicProbe(
                "LiDAR 원본", "/scan", 3,
                "/dev/ttyLIDAR와 'Now lidar is scanning' 로그를 확인하세요.",
            ),
            "imu": TopicProbe(
                "IMU", "/imu", 3,
                "/dev/ttyIMU 연결, 57600 baud, IMU 드라이버 로그를 확인하세요.",
            ),
            "vesc": TopicProbe(
                "VESC", "/vehicle/vesc_state", 3,
                "/dev/ttyMOTOR, 모터 배터리, VESC 텔레메트리를 확인하세요.",
            ),
        ...
        self._check_device(
            ...
            "LiDAR USB와 udev 별칭 /dev/ttyLIDAR를 확인하세요.",
        )
        self._check_device(
            "IMU USB",
            str(self.get_parameter("imu_device").value),
            "IMU USB와 udev 별칭 /dev/ttyIMU를 확인하세요.",
        )
        self._check_device(
            "VESC USB",
