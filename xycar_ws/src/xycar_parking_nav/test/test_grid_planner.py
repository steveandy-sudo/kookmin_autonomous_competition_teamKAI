...
from xycar_parking_nav.grid_planner import (
    MapGrid,
    astar,
    line_is_free,
    load_map_grid,
...
def test_parking_map_and_waypoint_route_config_exist():
    package = Path(__file__).resolve().parents[1]
    assert (package / "maps" / "parking_map.yaml").is_file()
    assert (package / "config" / "waypoint_route.yaml").is_file()
