...
if [[ ! -x "$WORKSPACE/src/my_drive/scripts/run_space_hybrid_test.sh" ]]; then
  if [[ -n "${XYCAR_WS:-}" ]]; then
...
check_duplicate_nodes() {
  ...
  for node in /xycar_wide_camera /xycar_lidar_node /xycar_vesc_driver; do
...
setsid ros2 launch my_drive basic_launch.py \
  ...
  motor_drive_enabled:=true >"$SENSOR_LOG" 2>&1 &
SENSOR_PID=$!
...
"$WORKSPACE/src/my_drive/scripts/run_space_hybrid_test.sh" \
