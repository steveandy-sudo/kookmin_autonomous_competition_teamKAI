...
if [[ ! -f "$WORKSPACE/install/setup.bash" ]] || \
  [[ ! -f "$WORKSPACE/src/my_drive/launch/basic_launch.py" ]]; then
  if [[ -n "${XYCAR_WS:-}" ]]; then
...
DIRECT_BEV_MODEL_PATH="${DIRECT_BEV_MODEL_PATH:-$WORKSPACE/src/my_lane/models/best_512.onnx}"
...
case "$RULE_PERCEPTION_BACKEND" in
  canonical)
    if [[ ! -f "$WORKSPACE/src/my_lane/launch/$LANE_PERCEPTION_LAUNCH" ]]; then
...
ros2 run my_drive space_drive_gate --ros-args \
