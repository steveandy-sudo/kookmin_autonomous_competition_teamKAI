"""Shared launch builder for the two supported real-vehicle profiles."""
...
def generate_drive_launch(*, integrated: bool) -> LaunchDescription:
    ...
    camera_launch = PathJoinSubstitution(
        [FindPackageShare("xycar_camera"), "launch", "xycar_camera.launch.py"]
    )
    lidar_launch = PathJoinSubstitution(
        [FindPackageShare("xycar_lidar"), "launch", "xycar_lidar.launch.py"]
    )
    vesc_launch = PathJoinSubstitution(
        [
    ...
    drive_enabled = LaunchConfiguration("motor_drive_enabled")
    selector_drive_enabled = False if integrated else drive_enabled
    integrated_condition = "true" if integrated else "false"
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "motor_drive_enabled", default_value="false"
            ),
            ...
            DeclareLaunchArgument(
                ...
                description=(
                    "my_lane perception launch; defaults to the "
            ...
            Node(
                ...
                parameters=[
                    object_config,
                    {
                        ...
                        "startup_signal_hsv_enabled": integrated,
            ...
            Node(
                ...
                condition=IfCondition(LaunchConfiguration("start_selector")),
                output="screen",
                parameters=[
                    hybrid_config,
                    {
                        "drive_enabled": ParameterValue(
                            selector_drive_enabled, value_type=bool
                        ),
            ...
            Node(
                package="my_drive",
                executable="traffic_shortcut_gate",
                name="traffic_shortcut_gate",
                condition=IfCondition(integrated_condition),
                output="screen",
                parameters=[
                    {
                        "drive_enabled": ParameterValue(
