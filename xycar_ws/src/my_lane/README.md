...
colcon build --packages-up-to my_lane --symlink-install
...
```bash
ros2 launch my_lane lane_seg_lraspp_low_latency_real.launch.py
```

Use the Gazebo-matched camera and BEV profile in simulation:

```bash
ros2 launch my_lane lane_seg_lraspp_sim_canonical.launch.py
```
...
```bash
ros2 launch my_drive basic_launch.py \
...
XYCAR_LANE_PERCEPTION_LAUNCH=lane_seg_row_centerline_low_latency_real.launch.py \
  ./install/my_drive/share/my_drive/scripts/run_space_hybrid_test.sh
```
...
```bash
ros2 run my_lane train_row_centerline \
...
ros2 launch my_drive basic_launch.py \
  ...
  canonical_forward_range_m:=2.5 motor_drive_enabled:=false
```
...
```bash
ros2 launch my_lane lane_seg_lraspp_canonical_only.launch.py \
...
```bash
rviz2 -d "$(ros2 pkg prefix --share my_control)/rviz/real_yolo_camera_canonical.rviz"
```
...
```bash
ros2 run my_lane export_lraspp_bag_montages \
  /path/to/rosbag \
  --sample-hz 10
```
...
| `models/model_centerline_xbin.pt` | Yellow-only 72-row x-bin model with gated mask adapter | `fb70ed54bc30ad3cf50d21f1baaa58ca1d1da31ffcbcc8a10d305a007dd71bbc` |
...
owned by `my_road/models`; local copies under this package are test
