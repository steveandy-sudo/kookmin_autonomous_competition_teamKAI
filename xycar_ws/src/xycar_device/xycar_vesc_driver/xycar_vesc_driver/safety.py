...
class VoltageGuard:
    ...
    def update(self, voltage: float, fault_code: int, now: float) -> str:
        ...
        if fault_active or voltage_stop:
            if fault_active:
                if fault_code != UNDER_VOLTAGE_FAULT_CODE:
                    self.manual_clear_required = True
                elif self.state != LATCHED:
                    self.manual_clear_required = False
            elif self.state != LATCHED:
                self.manual_clear_required = False
            self.state = LATCHED
        ...
        if self.state == LATCHED:
            if math.isfinite(voltage) and voltage >= self.recovery_voltage:
                if self.recovery_since is None:
                    ...
                    self.auto_recover
                    and not self.manual_clear_required
...
def slew(
    ...
    reversing = (
        current != 0.0
        and target != 0.0
        and math.copysign(1.0, target) != math.copysign(1.0, current)
    )
    if reversing:
        maximum_change = max(0.0, deceleration_limit) * dt
        if abs(current) <= maximum_change:
            return 0.0
        return current - math.copysign(maximum_change, current)
