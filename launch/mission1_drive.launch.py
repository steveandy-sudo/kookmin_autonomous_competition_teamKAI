# 과제 1 주행에 필요한 ROS2 노드와 파라미터를 한 번에 실행하는 launch 파일이다.
# CNN 조향 노드와 통합 주행 노드를 함께 실행하고, 모델 경로와 미션별 속도/판단 파라미터를 설정한다.
...
# launch 실행 시 사용할 모든 노드, 토픽명, 모델 경로, 미션 파라미터를 구성한다.
def generate_launch_description():
    # 빌드 후 설치된 track_drive 패키지에서 모델과 launch 리소스 경로를 찾는다.
    package_share = FindPackageShare('track_drive')
    default_cone_model_path = PathJoinSubstitution([
        package_share,
        'assets',
        'models',
        'cnn_steering_model.pt',
    ])
    default_yolo_model_path = PathJoinSubstitution([
        package_share,
        'assets',
        'models',
        'object_detector_model.onnx',
    ])
    # 아래 LaunchConfiguration들은 실행 시 인자로 덮어쓸 수 있어 현장 튜닝이 가능하다.
    ...
    # LaunchDescription에는 환경 변수, 실행 인자, 실제 ROS2 노드 실행 설정을 순서대로 담는다.
    return LaunchDescription([
        # ROS 로그 버퍼링을 줄여 주행 중 상태 메시지가 터미널에 바로 보이게 한다.
        SetEnvironmentVariable(
            name='LD_LIBRARY_PATH',
            value=[nvidia_library_path, ':', EnvironmentVariable('LD_LIBRARY_PATH', default_value='')],
        ),
        # 센서/모터 토픽 인자는 시뮬레이터 토픽 이름이 바뀌어도 launch 인자만으로 대응하기 위해 둔다.
        ...
        # 어린이보호구역 파라미터는 노면 표식 검출, 속도 제한, yellow centerline 추종을 함께 조정한다.
        DeclareLaunchArgument('school_zone_speed', default_value='17.0'),
        ...
        # 교차로 파라미터는 신호등, 정지선, 좌측 라바콘 유무를 조합해 직진/좌회전 경로를 고른다.
        ...
        # YOLO 안전 인식 인자는 보행자/신호등/차량 후보 검출의 모델 경로와 threshold를 조정한다.
        ...
        # 정지선 BEV 인자는 흰색 가로선의 폭/채움률/거리 기준을 조절한다.
        ...
        # CNN 조향 모델을 실행하되, 메인 노드의 enable/속도 제한 토픽으로 제어한다.
        Node(
            package='track_drive',
            executable='ai_drive_switch',
            name='cone_ai_direct',
            output='screen',
            parameters=[{
        ...
        # 카메라/라이다/객체 인식 결과를 종합해 최종 주행 명령을 결정하는 메인 노드이다.
        Node(
            package='track_drive',
