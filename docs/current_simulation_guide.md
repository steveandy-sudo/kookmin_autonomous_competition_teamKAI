...
카메라는 실차 자료를 따라 `1280x1024`, `30 Hz`, equidistant fisheye 모델과
보정 후 유효 HFOV 약 `102.95 deg`를 사용한다. LiDAR는 `505 samples`, `10 Hz`,
`-180~180 deg`, `0.1~12 m`를 기준으로 한다. 카메라 아래에는 실제 영상처럼
앞쪽으로 튀어나온 둥근 LiDAR만 보이도록 구성했다.
