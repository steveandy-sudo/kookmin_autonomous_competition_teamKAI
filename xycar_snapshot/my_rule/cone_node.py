...
class ConeNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("cone_min_drive_speed", 9.5)
        self.declare_parameter("cone_speed_steer_exponent", 1.0)
        self.declare_parameter("cone_speed_confidence_floor_ratio", 0.35)
        # Keep the vehicle-tested 9.5~17 profile for bends. A higher target is
    ...
    def pure_pursuit_at_distance(self, path: Sequence[Point2], lookahead: float) -> float:
        ...
        # Publish the physical target wheel angle. rule_driver performs the
        # measured Xycar steering-servo command conversion exactly once.
