...
class SequenceEntryNode(Node):
        ...
        self.selector = SequenceAwareEntrySelector(
            SequenceEntryConfig(
                ...
                w1_acquisition_required_frames=int(
                    self.get_parameter("w1_acquisition_required_frames").value
                ),
                w1_acquisition_window_frames=int(
                    self.get_parameter("w1_acquisition_window_frames").value
                ),
    ...
    def try_process(self) -> None:
        ...
        if not self.steering_started:
            entry_trigger_ready = w1_entry_trigger_ready(
                ...
                start_on_intersection=bool(
                    self.get_parameter("w1_start_on_intersection").value
                ),
            )
        ...
        if self.w1_spatial_gate_latched and rule_driving and w1_delay_ready:
            if not self.steering_started:
                self.get_logger().warning(
                    f"{MAGENTA}[MISSION] RED W1/W2 INTERSECTION -> DIRECT W1 "
                    "STEERING START: "
                    ...
                    f"entry_speed={entry_speed_command:.2f}{RESET}"
                )
                self.entry_progress_m = 0.0
                self.entry_steering_active_sec = 0.0
                self.entry_progress_update_time = observation_time
        ...
        if y1_visible and result.y1 is not None:
            ...
            self.get_logger().info(
                ...
                f"active={self.entry_steering_active_sec:.2f}s"
            )
