...
class XycarVescDriver(Node):
    ...
    def __init__(self) -> None:
        ...
        self._firmware_compatible = False
        ...
        for _ in range(100):
            ...
            if payload[0] == COMM_FW_VERSION and len(payload) >= 3:
                ...
                if version != self._firmware_version:
                    ...
                    expected = (
                        int(self.expected_firmware_major),
                        int(self.expected_firmware_minor),
                    )
                    self._firmware_compatible = (
                        version == expected
                        or bool(self.allow_firmware_mismatch)
                    )
                    if self._firmware_compatible:
                        self.get_logger().info(
                            "Connected to VESC firmware "
                            f"{version[0]}.{version[1]}"
                        )
                    else:
                        self.get_logger().error(
                            "VESC firmware mismatch: found "
                            f"{version[0]}.{version[1]}, expected "
                            f"{expected[0]}.{expected[1]}; motor output "
                            "remains disabled"
                        )
    ...
    def _publish_diagnostics(self) -> None:
        ...
        elif not self._firmware_compatible:
            status.level = DiagnosticStatus.ERROR
            status.message = "unsupported VESC firmware"
        ...
        values = {
            ...
            "firmware": firmware,
            "firmware_compatible": str(self._firmware_compatible),
