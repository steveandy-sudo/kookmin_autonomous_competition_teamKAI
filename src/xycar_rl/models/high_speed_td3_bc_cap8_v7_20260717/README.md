...
```bash
MODEL_DIR="$(ros2 pkg prefix xycar_rl)/share/xycar_rl/models/high_speed_td3_bc_cap8_v7_20260717"
```
...
Gazebo cap `8.0`, seed `20260724`: off track at `18.90m`, CTE `0.429m`.

This checkpoint is not approved. Use it only as a DAgger learner candidate.
...
ros2 launch xycar_rl real_shadow.launch.py \
  policy_kind:=camera_speed_td3_bc \
  checkpoint_path:=$MODEL_DIR/camera_speed_td3_bc_epoch_057.pth \
  min_speed_command:=4.0 max_speed_command:=12.0 \
  deployment_speed_cap:=4.0 \
  drive_enabled:=false lidar_safety_enabled:=false device:=cpu
```
