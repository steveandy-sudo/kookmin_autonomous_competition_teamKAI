...
if (( $# > 1 )); then
  echo "ERROR: only one positional value (speed) is accepted." >&2
  echo "Other driving parameters now use the verified defaults." >&2
  exit 2
fi
...
PURE_PURSUIT_CONTROL_X_M="${PURE_PURSUIT_CONTROL_X_M:--0.08}"
STANLEY_CONTROL_X_M="${STANLEY_CONTROL_X_M:-0.16}"
STANLEY_GAIN="${STANLEY_GAIN:-1.30}"
STANLEY_SOFTENING_MPS="${STANLEY_SOFTENING_MPS:-0.35}"
STRAIGHT_STANLEY_PERCENT="${STRAIGHT_STANLEY_PERCENT:-90.0}"
STRAIGHT_STANLEY_GAIN="${STRAIGHT_STANLEY_GAIN:-0.50}"
STRAIGHT_STANLEY_SOFTENING_MPS="${STRAIGHT_STANLEY_SOFTENING_MPS:-0.65}"
OPPOSED_STANLEY_PERCENT="${OPPOSED_STANLEY_PERCENT:-70.0}"
CONTROL_LATENCY_PREVIEW_SEC="${CONTROL_LATENCY_PREVIEW_SEC:-0.20}"
CURVE_CONTROL_LATENCY_PREVIEW_SEC="${CURVE_CONTROL_LATENCY_PREVIEW_SEC:-0.35}"
CURVE_CONTROL_LATENCY_MINIMUM_HOLD_SEC="${CURVE_CONTROL_LATENCY_MINIMUM_HOLD_SEC:-0.50}"
...
ADAPTIVE_STEERING_SPEED_ENABLED="${ADAPTIVE_STEERING_SPEED_ENABLED:-true}"
STEERING_TURN_SPEED_COMMAND="${STEERING_TURN_SPEED_COMMAND:-}"
STEERING_SLOWDOWN_START_ANGLE="${STEERING_SLOWDOWN_START_ANGLE:-18.0}"
STEERING_FULL_SLOWDOWN_ANGLE="${STEERING_FULL_SLOWDOWN_ANGLE:-42.0}"
...
# All tuning values except speed use verified defaults. Environment overrides
# remain available for controlled experiments without adding terminal prompts.
...
prompt_float OVERALL_SPEED_LIMIT_COMMAND \
  "전체 최종 속도 상한 command" "$SPEED_COMMAND" 0.0 30.0
...
prompt_float SHORTCUT_ENTRY_SPEED_COMMAND \
  "지름길 탐색·진입 속도 command" 9.0 0.0 30.0
prompt_float S_CURVE_ENTRY_SPEED_CAP_COMMAND \
  "지름길/green_car 이후 S자 진입 보호 속도 command" 20.0 0.0 30.0
prompt_float S_CURVE_ENTRY_RED_CAR_SPEED_CAP_COMMAND \
  "red_car 이후 좌회전·S자 진입 보호 속도 command" 13.0 0.0 30.0
prompt_float CONE_APPROACH_FIRST_SPEED_COMMAND \
  "라바콘 최초 감지 접근 속도 command" 15.0 0.0 30.0
prompt_float CONE_SPEED_COMMAND \
  "라바콘 확정·구간 주행 속도 command" 8.0 0.0 30.0
prompt_float VEHICLE_AVOIDANCE_SPEED_LIMIT_COMMAND \
  "기타 차량 회피 속도 상한 command" 20.0 0.0 30.0
prompt_float VEHICLE_RED_CAR_AVOIDANCE_SPEED_LIMIT_COMMAND \
  "red_car 회피 속도 상한 command" 20.0 0.0 30.0
prompt_float VEHICLE_GREEN_CAR_AVOIDANCE_SPEED_LIMIT_COMMAND \
  "green_car 회피 속도 상한 command" 20.0 0.0 30.0
prompt_float VEHICLE_AVOIDANCE_RIGHT_SETTLE_SPEED_LIMIT_COMMAND \
  "AVOID_RIGHT 차선 정착 전 속도 상한 command" 14.0 0.0 30.0
...
export SHORTCUT_ENTRY_SPEED_COMMAND
export S_CURVE_ENTRY_SPEED_CAP_COMMAND
export S_CURVE_ENTRY_RED_CAR_SPEED_CAP_COMMAND
export CONE_APPROACH_FIRST_SPEED_COMMAND CONE_SPEED_COMMAND
export VEHICLE_AVOIDANCE_SPEED_LIMIT_COMMAND
export VEHICLE_RED_CAR_AVOIDANCE_SPEED_LIMIT_COMMAND
export VEHICLE_GREEN_CAR_AVOIDANCE_SPEED_LIMIT_COMMAND
export VEHICLE_AVOIDANCE_RIGHT_SETTLE_SPEED_LIMIT_COMMAND
