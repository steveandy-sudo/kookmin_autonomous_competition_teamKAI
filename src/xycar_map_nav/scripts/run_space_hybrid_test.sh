...
if [[ ! -f "$WORKSPACE/src/xycar_map_nav/launch/real_sequential_hybrid_drive.launch.py" ]]; then
  if [[ -n "${XYCAR_WS:-}" ]]; then
...
S_CURVE_ENTRY_SPEED_CAP_COMMAND="${S_CURVE_ENTRY_SPEED_CAP_COMMAND:-20.0}"
S_CURVE_ENTRY_RED_CAR_SPEED_CAP_COMMAND="${S_CURVE_ENTRY_RED_CAR_SPEED_CAP_COMMAND:-13.0}"
S_CURVE_ENTRY_SPEED_CAP_START_DISTANCE_M="${S_CURVE_ENTRY_SPEED_CAP_START_DISTANCE_M:-8.0}"
...
DEGRADED_PATH_MINIMUM_SPAN_M="${DEGRADED_PATH_MINIMUM_SPAN_M:-0.40}"
DEGRADED_PATH_RELEASE_SPAN_M="${DEGRADED_PATH_RELEASE_SPAN_M:-0.60}"
DEGRADED_PATH_CONFIRMATION_FRAMES="${DEGRADED_PATH_CONFIRMATION_FRAMES:-2}"
DEGRADED_PATH_RELEASE_FRAMES="${DEGRADED_PATH_RELEASE_FRAMES:-2}"
...
CONE_SPEED_COMMAND="${CONE_SPEED_COMMAND:-8.0}"
CONE_APPROACH_FIRST_SPEED_COMMAND="${CONE_APPROACH_FIRST_SPEED_COMMAND:-15.0}"
CONE_APPROACH_YOLO_MIN_CONFIDENCE="${CONE_APPROACH_YOLO_MIN_CONFIDENCE:-0.40}"
CONE_APPROACH_CONFIRMED_DISTANCE_M="${CONE_APPROACH_CONFIRMED_DISTANCE_M:-1.15}"
CONE_SENSOR_PRESENCE_TIMEOUT_SEC="${CONE_SENSOR_PRESENCE_TIMEOUT_SEC:-0.25}"
CONE_EXIT_ABSENCE_SEC="${CONE_EXIT_ABSENCE_SEC:-0.25}"
RULE_TO_CONE_STEERING_RATE_COMMAND_PER_SEC="${RULE_TO_CONE_STEERING_RATE_COMMAND_PER_SEC:-180.0}"
...
RACE_LAP_POLICY_ENABLED="${XYCAR_RACE_LAP_POLICY_ENABLED:-false}"
...
VEHICLE_GREEN_CAR_RETRIGGER_REQUIRED_FRAMES="${VEHICLE_GREEN_CAR_RETRIGGER_REQUIRED_FRAMES:-2}"
VEHICLE_YOLO_TIMEOUT_SEC="${VEHICLE_YOLO_TIMEOUT_SEC:-0.50}"
VEHICLE_RED_CAR_YOLO_TIMEOUT_SEC="${VEHICLE_RED_CAR_YOLO_TIMEOUT_SEC:-0.50}"
...
VEHICLE_LEFT_OFFSET_M="${VEHICLE_LEFT_OFFSET_M:-0.13}"
VEHICLE_RIGHT_OFFSET_M="${VEHICLE_RIGHT_OFFSET_M:-0.13}"
VEHICLE_OFFSET_RATE_MPS="${VEHICLE_OFFSET_RATE_MPS:-0.65}"
VEHICLE_ACTIVE_SIDE_RESELECTION_OFFSET_RATE_MPS="${VEHICLE_ACTIVE_SIDE_RESELECTION_OFFSET_RATE_MPS:-1.30}"
VEHICLE_AVOIDANCE_SPEED_LIMIT_COMMAND="${VEHICLE_AVOIDANCE_SPEED_LIMIT_COMMAND:-20.0}"
VEHICLE_AVOIDANCE_RIGHT_SETTLE_SPEED_LIMIT_COMMAND="${VEHICLE_AVOIDANCE_RIGHT_SETTLE_SPEED_LIMIT_COMMAND:-14.0}"
VEHICLE_AVOIDANCE_RIGHT_SETTLE_TOLERANCE_M="${VEHICLE_AVOIDANCE_RIGHT_SETTLE_TOLERANCE_M:-0.02}"
VEHICLE_AVOIDANCE_RIGHT_ENTRY_OPPOSED_STEERING_GUARD_SEC="${VEHICLE_AVOIDANCE_RIGHT_ENTRY_OPPOSED_STEERING_GUARD_SEC:-0.35}"
...
VEHICLE_GREEN_CAR_AVOIDANCE_SPEED_LIMIT_COMMAND="${VEHICLE_GREEN_CAR_AVOIDANCE_SPEED_LIMIT_COMMAND:-$VEHICLE_AVOIDANCE_SPEED_LIMIT_COMMAND}"
VEHICLE_MINIMUM_AVOID_SEC="${VEHICLE_MINIMUM_AVOID_SEC:-0.50}"
VEHICLE_CLEAR_HOLD_SEC="${VEHICLE_CLEAR_HOLD_SEC:-0.50}"
VEHICLE_GREEN_CAR_CLEAR_HOLD_SEC="${VEHICLE_GREEN_CAR_CLEAR_HOLD_SEC:-0.60}"
VEHICLE_RETURN_HOLD_SEC="${VEHICLE_RETURN_HOLD_SEC:-0.30}"
VEHICLE_RETURN_DEADBAND_M="${VEHICLE_RETURN_DEADBAND_M:-0.02}"
VEHICLE_RETURN_CROSS_TRACK_ERROR_M="${VEHICLE_RETURN_CROSS_TRACK_ERROR_M:-0.08}"
VEHICLE_RETURN_REQUIRED_FRAMES="${VEHICLE_RETURN_REQUIRED_FRAMES:-3}"
VEHICLE_RETURN_DIAGNOSTICS_TIMEOUT_SEC="${VEHICLE_RETURN_DIAGNOSTICS_TIMEOUT_SEC:-0.30}"
VEHICLE_AVOIDANCE_STRAIGHT_PP_WEIGHT="${VEHICLE_AVOIDANCE_STRAIGHT_PP_WEIGHT:-0.45}"
VEHICLE_RETURN_STRAIGHT_PP_WEIGHT="${VEHICLE_RETURN_STRAIGHT_PP_WEIGHT:-0.45}"
...
prompt_bool TRAFFIC_LIGHT_CONTROL_ENABLED \
  "Traffic-light control" true
...
if [[ "$STEERING_ONLY" == "true" ]]; then
  ...
  if [[ "$CURVATURE_SPEED_CONTROL_ENABLED" == "true" ]]; then
    ...
    degraded_default="$(awk -v speed="$SPEED_COMMAND" \
      'BEGIN { printf "%.3f", (speed < 15.0 ? speed : 15.0) }')"
...
prompt_float CONE_SENSOR_PRESENCE_TIMEOUT_SEC \
  "Cone sensor-loss hold [s]" 0.25 0.1 10.0
...
degraded_path_release_span_m: $DEGRADED_PATH_RELEASE_SPAN_M
degraded_path_confirmation_frames: $DEGRADED_PATH_CONFIRMATION_FRAMES
degraded_path_release_frames: $DEGRADED_PATH_RELEASE_FRAMES
...
s_curve_entry_speed_cap_start_distance_m: $S_CURVE_ENTRY_SPEED_CAP_START_DISTANCE_M
...
cone_approach_first_speed_command: $CONE_APPROACH_FIRST_SPEED_COMMAND
cone_approach_yolo_min_confidence: $CONE_APPROACH_YOLO_MIN_CONFIDENCE
cone_approach_confirmed_distance_m: $CONE_APPROACH_CONFIRMED_DISTANCE_M
cone_sensor_presence_timeout_sec: $CONE_SENSOR_PRESENCE_TIMEOUT_SEC
cone_exit_absence_sec: $CONE_EXIT_ABSENCE_SEC
rule_to_cone_steering_rate_command_per_sec: $RULE_TO_CONE_STEERING_RATE_COMMAND_PER_SEC
test_profile: "$TEST_PROFILE"
enable_rviz: $ENABLE_RVIZ
start_cone: $START_CONE
traffic_light_control_enabled: $TRAFFIC_LIGHT_CONTROL_ENABLED
race_lap_policy_enabled: $RACE_LAP_POLICY_ENABLED
...
vehicle_green_car_retrigger_required_frames: $VEHICLE_GREEN_CAR_RETRIGGER_REQUIRED_FRAMES
vehicle_active_side_reselection_offset_rate_mps: $VEHICLE_ACTIVE_SIDE_RESELECTION_OFFSET_RATE_MPS
...
vehicle_avoidance_right_settle_speed_limit_command: $VEHICLE_AVOIDANCE_RIGHT_SETTLE_SPEED_LIMIT_COMMAND
vehicle_avoidance_right_settle_tolerance_m: $VEHICLE_AVOIDANCE_RIGHT_SETTLE_TOLERANCE_M
vehicle_avoidance_right_entry_opposed_steering_guard_sec: $VEHICLE_AVOIDANCE_RIGHT_ENTRY_OPPOSED_STEERING_GUARD_SEC
vehicle_red_car_avoidance_speed_limit_command: $VEHICLE_RED_CAR_AVOIDANCE_SPEED_LIMIT_COMMAND
vehicle_green_car_avoidance_speed_limit_command: $VEHICLE_GREEN_CAR_AVOIDANCE_SPEED_LIMIT_COMMAND
vehicle_minimum_avoid_sec: $VEHICLE_MINIMUM_AVOID_SEC
vehicle_clear_hold_sec: $VEHICLE_CLEAR_HOLD_SEC
vehicle_green_car_clear_hold_sec: $VEHICLE_GREEN_CAR_CLEAR_HOLD_SEC
vehicle_return_hold_sec: $VEHICLE_RETURN_HOLD_SEC
vehicle_return_deadband_m: $VEHICLE_RETURN_DEADBAND_M
vehicle_return_cross_track_error_m: $VEHICLE_RETURN_CROSS_TRACK_ERROR_M
vehicle_return_required_frames: $VEHICLE_RETURN_REQUIRED_FRAMES
vehicle_return_diagnostics_timeout_sec: $VEHICLE_RETURN_DIAGNOSTICS_TIMEOUT_SEC
vehicle_avoidance_straight_pure_pursuit_weight: $VEHICLE_AVOIDANCE_STRAIGHT_PP_WEIGHT
vehicle_return_straight_pure_pursuit_weight: $VEHICLE_RETURN_STRAIGHT_PP_WEIGHT
...
for required_package in \
  xycar_map_nav xycar_rule_drive lane_seg_control my_rule; do
...
if [[ "$CURVATURE_SPEED_CONTROL_ENABLED" == "true" ]]; then
  ...
  echo "Degraded path latch: enter <${DEGRADED_PATH_MINIMUM_SPAN_M}m x${DEGRADED_PATH_CONFIRMATION_FRAMES}, release >=${DEGRADED_PATH_RELEASE_SPAN_M}m x${DEGRADED_PATH_RELEASE_FRAMES} (new Xbin frames)"
else
  echo "Path speed: OFF"
fi
if [[ "$ADAPTIVE_STEERING_SPEED_ENABLED" == "true" ]]; then
...
echo "Cone first-detection speed command: $CONE_APPROACH_FIRST_SPEED_COMMAND"
echo "Cone early-detection confidence: $CONE_APPROACH_YOLO_MIN_CONFIDENCE | command 8 near <=${CONE_APPROACH_CONFIRMED_DISTANCE_M}m"
...
echo "Cone continuous-absence exit: ${CONE_EXIT_ABSENCE_SEC}s"
echo "Traffic-light control: $TRAFFIC_LIGHT_CONTROL_ENABLED"
echo "Three-lap signal hardcoding: $RACE_LAP_POLICY_ENABLED"
...
echo "Avoidance recovery: green_car retrigger=${VEHICLE_GREEN_CAR_RETRIGGER_REQUIRED_FRAMES} frames | AVOID_RIGHT opposed-steering guard=${VEHICLE_AVOIDANCE_RIGHT_ENTRY_OPPOSED_STEERING_GUARD_SEC}s"
if [[ "$CONE_AS_VEHICLE_OBSTACLE" == "true" ]]; then
...
echo "Avoidance right settle: cap=${VEHICLE_AVOIDANCE_RIGHT_SETTLE_SPEED_LIMIT_COMMAND}, tolerance=${VEHICLE_AVOIDANCE_RIGHT_SETTLE_TOLERANCE_M}m | side-reselection rate=${VEHICLE_ACTIVE_SIDE_RESELECTION_OFFSET_RATE_MPS}m/s"
echo "Avoidance YOLO timeout: default/green_car=${VEHICLE_YOLO_TIMEOUT_SEC}s red_car=${VEHICLE_RED_CAR_YOLO_TIMEOUT_SEC}s"
echo "Avoidance clear hold: default/red_car=${VEHICLE_CLEAR_HOLD_SEC}s green_car=${VEHICLE_GREEN_CAR_CLEAR_HOLD_SEC}s"
echo "Avoidance steering: active straight PP=${VEHICLE_AVOIDANCE_STRAIGHT_PP_WEIGHT} (Stanley=$(awk -v w="$VEHICLE_AVOIDANCE_STRAIGHT_PP_WEIGHT" 'BEGIN { printf "%.2f", 1.0-w }'))"
echo "Avoidance return: |CTE|<=${VEHICLE_RETURN_CROSS_TRACK_ERROR_M}m x${VEHICLE_RETURN_REQUIRED_FRAMES} frames | straight PP=${VEHICLE_RETURN_STRAIGHT_PP_WEIGHT}"
echo "S-entry guard: $S_CURVE_ENTRY_GUARD_ENABLED | shortcut/green_car -> $S_CURVE_ENTRY_SPEED_CAP_COMMAND after curve detection or ${S_CURVE_ENTRY_SPEED_CAP_START_DISTANCE_M}m | red_car -> $S_CURVE_ENTRY_RED_CAR_SPEED_CAP_COMMAND immediately | RULE steering passthrough"
...
setsid ros2 launch xycar_map_nav real_sequential_hybrid_drive.launch.py \
  ...
  cone_speed_command:="$CONE_SPEED_COMMAND" \
  cone_approach_first_speed_command:="$CONE_APPROACH_FIRST_SPEED_COMMAND" \
  cone_approach_confirmed_speed_command:="$CONE_SPEED_COMMAND" \
  cone_approach_yolo_min_confidence:="$CONE_APPROACH_YOLO_MIN_CONFIDENCE" \
  cone_approach_confirmed_distance_m:="$CONE_APPROACH_CONFIRMED_DISTANCE_M" \
  cone_sensor_presence_timeout_sec:="$CONE_SENSOR_PRESENCE_TIMEOUT_SEC" \
  cone_exit_absence_sec:="$CONE_EXIT_ABSENCE_SEC" \
...
ros2 run xycar_map_nav space_drive_gate --ros-args \
  ...
  -p rule_to_cone_steering_rate_command_per_sec:="$RULE_TO_CONE_STEERING_RATE_COMMAND_PER_SEC" \
