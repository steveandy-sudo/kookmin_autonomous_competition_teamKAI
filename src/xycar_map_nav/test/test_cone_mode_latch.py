...
def approach_limit(**overrides):
    values = {
        ...
        "cluster_forward_distance_m": float("inf"),
        "confirmed_distance_m": 0.95,
...
def test_strong_first_yolo_keeps_early_approach_speed_until_near():
    assert approach_limit(
        yolo_frames=1,
        yolo_age_sec=0.1,
...
def test_confirmed_yolo_stays_at_approach_speed_until_near_cluster():
...
def test_near_fused_cluster_uses_validated_cone_speed():
    assert approach_limit(
        cluster_count=1,
        cluster_age_sec=0.1,
        cluster_forward_distance_m=0.94,
    ) == (
        8.0,
        "near_cluster",
    )
    assert approach_limit(
        ...
        cluster_count=1,
        cluster_age_sec=0.1,
        cluster_forward_distance_m=1.20,
    ) == (15.0, "first_yolo")
