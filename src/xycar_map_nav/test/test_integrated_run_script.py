...
def test_integrated_run_waits_for_measured_lane_center_on_avoidance_return() -> None:
    ...
    assert 'VEHICLE_AVOIDANCE_STRAIGHT_PP_WEIGHT:-0.45' in run_source
    ...
    assert 'vehicle_avoidance_straight_pure_pursuit_weight:="$VEHICLE_AVOIDANCE_STRAIGHT_PP_WEIGHT"' in run_source
    ...
    assert '"vehicle_avoidance_straight_pure_pursuit_weight"' in launch_source
...
def test_integrated_run_brakes_for_cones_earlier_and_holds_green_car_longer() -> None:
    run_source = RUN_SCRIPT.read_text(encoding="utf-8")
    launch_source = LAUNCH_FILE.read_text(encoding="utf-8")
    config_source = HYBRID_CONFIG.read_text(encoding="utf-8")

    assert 'CONE_APPROACH_CONFIRMED_DISTANCE_M:-1.15' in run_source
    assert 'VEHICLE_GREEN_CAR_CLEAR_HOLD_SEC:-0.60' in run_source
    assert 'cone_approach_confirmed_distance_m:="$CONE_APPROACH_CONFIRMED_DISTANCE_M"' in run_source
    assert 'vehicle_green_car_clear_hold_sec:="$VEHICLE_GREEN_CAR_CLEAR_HOLD_SEC"' in run_source
    assert '"cone_approach_confirmed_distance_m", default_value="1.15"' in launch_source
    assert '"vehicle_green_car_clear_hold_sec", default_value="0.60"' in launch_source
    assert "cone_approach_confirmed_distance_m: 1.15" in config_source
    assert "vehicle_green_car_clear_hold_sec: 0.60" in config_source
...
def test_integrated_run_defaults_to_25_12_15_profile() -> None:
    run_source = RUN_SCRIPT.read_text(encoding="utf-8")
    complete_source = COMPLETE_SCRIPT.read_text(encoding="utf-8")
    launch_source = LAUNCH_FILE.read_text(encoding="utf-8")
    ...
    for source in (run_source, complete_source):
        ...
        assert "speed < 15.0 ? speed : 15.0" in source
...
def test_integrated_run_exposes_right_avoidance_settling_controls() -> None:
    run_source = RUN_SCRIPT.read_text(encoding="utf-8")
    launch_source = LAUNCH_FILE.read_text(encoding="utf-8")
    hybrid_config = HYBRID_CONFIG.read_text(encoding="utf-8")
    ...
    assert '"vehicle_green_car_retrigger_required_frames",' in launch_source
    assert (
        '"vehicle_avoidance_right_entry_opposed_steering_guard_sec",'
        in launch_source
    )
    assert (
    ...
    assert "vehicle_green_car_retrigger_required_frames: 2" in hybrid_config
