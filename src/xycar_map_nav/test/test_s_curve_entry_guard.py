...
def test_shortcut_and_green_car_guards_delay_command_20_cap(trigger) -> None:
    ...
    assert not guard.state().speed_cap_active

    update(guard, angle=-2.0, speed=12.0)
...
def test_shortcut_guard_uses_distance_fallback_before_s_curve() -> None:
    ...
    while guard.state().distance_m < 8.0:
        update(guard, angle=0.0, speed=25.0, vehicle_speed=2.0)
...
def test_center_and_left_offset_right_corrections_are_preserved(
    ...
    assert (early_angle, early_speed) == (right_correction, 25.0)
...
def test_overdue_guard_stays_slow_instead_of_reaccelerating() -> None:
    guard = SCurveEntryGuard(
        SCurveEntryGuardConfig(
            speed_cap_start_distance_m=1.0,
            overdue_distance_m=1.0,
        )
    )
    guard.start(SCurveEntryTrigger.GREEN_CAR_EXIT)
