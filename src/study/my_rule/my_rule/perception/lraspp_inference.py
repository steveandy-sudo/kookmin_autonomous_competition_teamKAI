#!/usr/bin/env python3
"""ROS 2 TorchScript LR-ASPP lane perception without motor outputs."""
...
from my_rule.perception.canonical_adapter import (
    BevGeometry,
...
class LanePerceptionNode(Node):
    ...
    def __init__(self) -> None:
        super().__init__("my_rule_lane_perception_node")
        ...
        self.declare_parameter("yellow_yolo_assist_enabled", True)
        self.declare_parameter(
            "object_detections_topic", "/my_rule/object_detections"
        )
        self.declare_parameter(
            "yellow_yolo_class_name", "yellow_centerline"
        )
        self.declare_parameter("yellow_yolo_confidence", 0.45)
        self.declare_parameter("yellow_yolo_timeout_sec", 0.60)
        self.declare_parameter("yellow_yolo_horizontal_margin_px", 18)
        self.declare_parameter("yellow_yolo_minimum_pixels", 3)
        self.declare_parameter("yellow_yolo_minimum_retained_fraction", 0.25)
        self.declare_parameter(
            "yellow_yolo_minimum_vertical_span_fraction", 0.55
        )
        ...
        self.yellow_yolo_assist_enabled = bool(
            self.get_parameter("yellow_yolo_assist_enabled").value
        )
        self.yellow_yolo_hints: tuple[DetectionRecord, ...] = ()
        self.yellow_yolo_image_width = 0
        self.yellow_yolo_received_at = 0.0
        self.yellow_yolo_applied_count = 0
        ...
        self.object_detection_sub = None
        if self.yellow_yolo_assist_enabled:
        ...
        self.get_logger().info(
            ...
            f"yellow_yolo_assist={self.yellow_yolo_assist_enabled}, "
    ...
    def on_object_detections(
        ...
        target = normalize_class_name(
            str(self.get_parameter("yellow_yolo_class_name").value)
        )
        hints = []
        for detection in message.detections:
            if normalize_class_name(detection.class_name) != target:
                continue
            hints.append(
                DetectionRecord(
                    class_name=target,
        ...
        self.yellow_yolo_hints = tuple(hints)
        self.yellow_yolo_image_width = int(message.image_width)
        self.yellow_yolo_received_at = time.monotonic()
    ...
    def process_image(self, message: CameraMessage) -> None:
        ...
        hint_age = time.monotonic() - self.yellow_yolo_received_at
        if (
            self.yellow_yolo_assist_enabled
            and self.yellow_yolo_hints
            and hint_age
            <= self.parameter_float("yellow_yolo_timeout_sec")
        ):
            yellow_small, assisted = restrict_yellow_mask_to_hints(
                yellow_small,
                self.yellow_yolo_hints,
                image_width=self.yellow_yolo_image_width,
                class_name=str(
                    self.get_parameter("yellow_yolo_class_name").value
                ),
                confidence=self.parameter_float("yellow_yolo_confidence"),
                horizontal_margin_px=int(
                    self.get_parameter(
                        "yellow_yolo_horizontal_margin_px"
                    ).value
                ),
                minimum_pixels=int(
                    self.get_parameter("yellow_yolo_minimum_pixels").value
                ),
                minimum_retained_fraction=self.parameter_float(
                    "yellow_yolo_minimum_retained_fraction"
                ),
                minimum_vertical_span_fraction=self.parameter_float(
                    "yellow_yolo_minimum_vertical_span_fraction"
                ),
            )
            if assisted:
                self.yellow_yolo_applied_count += 1
        debug_requested = (
